...
public class runSingleSimilarityJoin {
	...
	public static void main(String[] args) {
		...
		querytables.add("freebase_company");
		querytables.add("dbpedia_company");
		querytables.add("sec_company");
		basetables.add("freebase_company");
		basetables.add("dbpedia_company");
		basetables.add("sec_company");
			
		Config config = new Config();
		MySqlDB mysqlDB = new MySqlDB(config.returnURL(), config.user, config.passwd);
		
		/*
		String resultTable = "single_sjoinresults";
		try {
		...
		*/
		...
		for (String querytable : querytables){
			for (String basetable : basetables){
				threads.add(new RunSimilarityJoinThread(querytable,basetable,bm25WeightedJaccard,0.3));				
			}
		}
